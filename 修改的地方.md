看到问题了！大部分推送失败，但没有显示具体错误。我们需要：

1. **查看详细错误信息**
2. **不删除 workflows**（你要求保留）

## 修改后的脚本：`scripts/sync.sh`
...
1. **✅ 保留 workflows** - `removeWorkflows: false`
2. **✅ 显示详细错误** - 推送失败时显示完整输出
3. **✅ 错误分类** - 识别具体错误原因
4. **✅ 退出码显示** - 显示 git 命令的退出码
5. **✅ 错误日志预览** - 在控制台显示前 50 行错误

## 部署更新：

```bash
git add config/repos.json scripts/sync.sh
git commit -m "Keep workflows and show detailed errors"
git push
```

运行后会显示详细的错误信息，帮助我们诊断问题！请运行一次并分享错误日志。